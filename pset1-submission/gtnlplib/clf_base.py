...
def make_feature_vector(base_features,label):
    ...
    '''
    feature_dict = {}
    for word in list(base_features):
        feature_dict[(label, word)] = base_features[word]
...
def predict(base_features,weights,labels):
    ...
    for label, feature in list(weights.keys()):
        if base_features[feature]:
            scores[label] += weights[(label, feature)] * base_features[feature]
        elif feature == OFFSET:
            scores[label] += weights[(label, feature)]
